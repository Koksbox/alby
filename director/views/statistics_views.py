...
def director_manager_statistic(request, user_id):
    ...
    user = get_object_or_404(CustomUser, id=user_id)
    logger.info(f'Пользователь {request.user} просматривает статистику менеджера {user.full_name}')
    ...
    # Статистика по созданным задачам
    ...
    # Статистика по проверенным задачам
    reviewed_tasks = TaskReview.objects.filter(
        user=user,
        task__created_at__gte=first_day_of_month,
        task__created_at__lt=last_day_of_month
    ).count()
    ...
    try:
        ...
        logger.error(f'Ошибка при расчете времени работы для менеджера {user.full_name}: {str(e)}')
        total_hours = 0
    ...
    try:
        ...
        logger.error(f'Ошибка при расчете зарплаты для менеджера {user.full_name}: {str(e)}')
        total_salary = 0

    logger.info(f'Статистика для менеджера {user.full_name} успешно сформирована за {selected_month.strftime("%Y-%m")}')
